...
### Changed
- Moved Python dependency requirement files from root-level `requirements-*.txt`
  into the `requirements/` directory while keeping legacy runtime fallbacks for
  older bundles.
...
### Fixed
- External API calls now send a stable `User-Agent`, avoiding Groq gateway 403/1010 blocks from Python's default `urllib` client.
...
- Settings UI dictionary suggestion preview/apply flow for benchmark/history JSONL.
