...
| `device` | `VOICEPI_DEVICE` | `auto` | Restart | Compute device for local STT: auto picks a GPU if present, else CPU; force with cuda or cpu. **The `cuda` value is only accepted on binaries built with a whisper.cpp GPU backend (`--features whisper-rs-vulkan`, `--features whisper-rs-cuda`, or the GPU installer).** On a CPU-only build the Settings UI hides `cuda` and `whisper-dictate config set device cuda` fails validation — otherwise Whisper would silently demote to CPU. Configs that carry `device = "cuda"` from an older UI are auto-coerced to `auto` on load with a one-line warning. |
...
| `--device D` | `$VOICEPI_DEVICE` | `auto` \| `cuda` \| `cpu` | Compute device for this run. `cuda` is only honoured by binaries built with `--features whisper-rs-vulkan` or `--features whisper-rs-cuda` (or the GPU installer); on a CPU-only binary the option is refused rather than silently demoting to CPU. |
