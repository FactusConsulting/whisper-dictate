...
| `postprocess_profiles` | `VOICEPI_POSTPROCESS_PROFILES` | _(unset)_ | Live | JSON array of postprocess profiles the second hotkey cycles through: each entry has name, processor, mode, model, base_url, api_key, timeout_ms, max_input_chars, max_output_chars, redact, redact_terms. Empty falls back to the built-in grammar/email/bullets seed. |
