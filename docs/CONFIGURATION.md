...
| **Push-to-talk key** | Speech | `VOICEPI_KEY` | `--key` | `ctrl_r` | pynput key name (`ctrl_r`, `alt_r`, `f9`, …) or `a+b` chord | hold-to-talk key. When this is a bare modifier (ctrl/shift/alt/win variant) it activates only when pressed **alone** — pressing it as part of a shortcut (e.g. Shift+Ctrl) does not start dictation, and a key pressed while it is held (e.g. Ctrl then C) cancels and discards the clip. |
...
| `VOICEPI_KEY` | `ctrl_r` | pynput key name, or chord `a+b` | Hold-to-talk key. e.g. `ctrl_r`, `alt_r`, `shift_r`, `f9`, or `shift_r+ctrl_r` (hold both). A bare-modifier key (ctrl/shift/alt/win variant) activates only when pressed alone — not inside a shortcut chord. Also `--key`. |
...
|---|---|---|---|
| `--key` | `$VOICEPI_KEY` or `ctrl_r` | pynput key name, or chord `a+b` | Hold-to-talk key. e.g. `ctrl_r`, `alt_r`, `shift_r`, `f9`, or `shift_r+ctrl_r` (hold both). A bare-modifier key activates only when pressed alone (not inside a shortcut chord). |
