...
| `transcribe-file` | ✅ | | Public one-shot 16 kHz mono WAV transcription through the configured local whisper.cpp or OpenAI-compatible cloud backend. Text or JSON output; no Python fallback. Available in Rust-controller distributions; the current legacy Nix derivation does not expose it. |
