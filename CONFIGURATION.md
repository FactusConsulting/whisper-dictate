...
| `VOICEPI_POST_API_KEY` / `OPENAI_API_KEY` | *(unset)* | API key | Bearer token for `VOICEPI_POST_PROCESSOR=openai`. `VOICEPI_POST_API_KEY` takes precedence over `OPENAI_API_KEY` for post-processing. API keys are read from env only and are not saved by the Settings UI. |
