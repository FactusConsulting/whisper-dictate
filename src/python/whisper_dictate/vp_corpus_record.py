...
Never raises out: every failure (unknown id, missing device, no corpus,
sounddevice unavailable) is reported as a single ``corpus_record_error`` JSON
line on stdout and the worker exits 0. The UI parses the start/progress/done/error
lines.
"""
