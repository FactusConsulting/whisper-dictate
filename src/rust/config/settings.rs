...
impl AppSettings {
    ...
    pub(crate) fn setting_value(&self, key: &str) -> Option<&str> {
        match key {
            ...
            "xkb_layout" => Some(&self.xkb_layout),
            "preview_seconds" => Some(&self.preview_seconds),
            "audio_ducking" => Some(if self.audio_ducking { "1" } else { "0" }),
            "audio_ducking_level" => Some(&self.audio_ducking_level),
            "json_output" => Some(if self.inject_json { "1" } else { "0" }),
            "metrics_jsonl" => Some(&self.metrics_jsonl),
            "history_enabled" => Some(if self.history_enabled { "1" } else { "0" }),
            "history_jsonl" => Some(&self.history_jsonl),
