...
impl WhisperDictateApp {
    ...
    pub(in crate::ui) fn system_tab(&mut self, ui: &mut egui::Ui) {
        ...
        // Everything below except Appearance (ui_theme/ui_language) is
        // advanced-only in Simple mode: maintenance actions, the benchmark
        // and corpus-record tools, Display, Feedback, Updates, Diagnostics,
        // and Integration are all advanced settings/tools with no single
        // "essential" row to keep, so the whole cluster is hidden.
        if mode == SettingsMode::Advanced {
            ...
            ui.separator();
            ui.add_space(8.0);
        } // mode == SettingsMode::Advanced (Maintenance cluster)

        // --- Appearance: theme + UI language. Always shown — the only
        // System settings kept in Simple mode (see settings_mode.rs).
        ui.horizontal_wrapped(|ui| {
            section_label(
                ui,
        ...
        // Display, Feedback, Updates, Diagnostics, and Integration are all
        // advanced settings — hidden in Simple mode (only Appearance, above,
        // stays visible there).
        if mode == SettingsMode::Advanced {
            ui.add_space(10.0);
            ui.horizontal_wrapped(|ui| {
        ...
        } // mode == SettingsMode::Advanced (Display/Feedback/Updates/Diagnostics/Integration)
    }
