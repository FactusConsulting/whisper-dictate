...
- Treat Windows as the primary supported desktop path. Changes to launchers,
  installers, subprocess handling, console encoding, Settings UI behavior, and
  keyboard/text injection must be reviewed for Windows behavior, not just
  platform-neutral Python logic.
- Use the local installer loop for internal Windows testing. When changing
  installer files, `setup.ps1`, `settings-ui.ps1`, shortcuts, bundled files, or
  Windows launch behavior, build a local installer with
  `scripts/build-windows-installer.ps1` and report the generated `Output\*.exe`.
...
- Preserve the Windows unified controller model. The Settings UI owns the
  managed runtime process on Windows, must avoid duplicate UI instances, and
  must make start, stop, restart, and required restarts explicit.
