...
section "config get/set (persistence roundtrip)"
    ...
    # The in-process Nemotron path is intentionally config-only here: a
    # hermetic Wayland smoke must not download the 700+ MB model or contact
    # NVIDIA. Verify that the shipped CLI accepts the zero-config provider
...
warn "Simple mode's tab/field hiding is GUI rendering, not checked headlessly -- verify manually: Settings -> Simple hides Quality/Dictionary/Profiles and shows only the essential fields on each remaining tab; the Post tab IS shown in Simple with exactly its Post processor + Post mode rows (plus the Post API key block once a cloud processor is picked) and no empty group box or dangling heading, and 'Reset page' there in Simple resets only those rows; with a Custom cloud provider the API URL row still shows in Simple (it has no other way to be set); the compact strip's Dictionary button still lands somewhere usable (Speech) when Dictionary is hidden"
