...
settings_schema.json (shared/config/settings_schema.json) is the
...
SCHEMA_PATH = REPO_ROOT / "shared" / "config" / "settings_schema.json"
...
def render_block(settings: list[dict]) -> str:
    ...
    lines.append(
        "_Generated from "
        "`shared/config/settings_schema.json` by "
