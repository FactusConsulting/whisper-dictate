          ...
          default = pkgs.mkShell {
            packages = with pkgs; [
              (python3.withPackages (ps: with ps; [
                faster-whisper requests numpy sounddevice pynput pyperclip
              ] ++ nixpkgs.lib.optionals stdenv.isLinux [ evdev scipy ]))
            ];
          };
        }
      );
