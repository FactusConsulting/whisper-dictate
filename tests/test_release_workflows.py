...
class RustReleaseWorkflowTests(unittest.TestCase):
    ...
    def test_homebrew_formula_installs_linux_release_bundle(self):
        ...
        self.assertIn('Pathname.new(home)/".local/share/applications/whisper-dictate.desktop"', bump_step)
        self.assertIn('Pathname.new(home)/".config/autostart/whisper-dictate.desktop"', bump_step)
        self.assertIn('Exec=#{exe} ui', bump_step)
